...
```
    ...
    - Tool/progress rows render as compact emoji + title + detail when available, for example `ðŸ› ï¸?Bash: run tests` or `ðŸ”Ž Web Search: for "query"`.
...
- In `agent-proxy` mode, forced consult fallback skips likely incomplete transcript fragments such as text ending in `...` or a trailing connector like `and`, plus obvious non-actionable closings like â€œbe right backâ€?or â€œbyeâ€? Logs show `forced agent consult skipped reason=...` when this prevents a stale queued answer.
