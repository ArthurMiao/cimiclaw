...
- cached credentials stay under `~/.cimiclaw/credentials/matrix/`
- runtime state stays under `~/.cimiclaw/matrix/`
...
- OpenClaw writes a marker file at `~/.cimiclaw/matrix/migration-snapshot.json` after a successful snapshot so later startup and repair passes can reuse the same archive.
...
- the oldest flat Matrix store (`~/.cimiclaw/matrix/bot-storage.json` and `~/.cimiclaw/matrix/crypto/`) came from a single-store layout, so OpenClaw can only migrate it into one resolved Matrix account target
