...
ðŸ§© Active Memory: status=ok elapsed=842ms query=recent summary=34 chars
ðŸ”Ž Active Memory Debug: Lemon pepper wings with blue cheese.
```
...
<AccordionGroup>
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Turn on `/verbose on` to also see the `ðŸ§© Active Memory: ...` status line
      after each reply.
