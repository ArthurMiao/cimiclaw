...
| Manifest ownership   | `cimiclaw.plugin.json` | Declares the backend id before runtime loads              |
...
<Check>`cimiclaw.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>
