...
- `~/.cimiclaw/sandbox/containers.json`
- `~/.cimiclaw/sandbox/browsers.json`
...
Sandbox settings live in `~/.cimiclaw/cimiclaw.json` under `agents.defaults.sandbox` (per-agent overrides go in `agents.list[].sandbox`):
