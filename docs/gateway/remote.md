...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  ...
  break-glass. There is no `cimiclaw.json` equivalent; this must be process
...
Save this as `~/Library/LaunchAgents/ai.cimiclaw.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>ai.cimiclaw.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/ai.cimiclaw.ssh-tunnel.plist
```

The tunnel will start automatically at login, restart on crash, and keep the forwarded port live.

<Note>
If you have a leftover `com.cimiclaw.ssh-tunnel` LaunchAgent from an older setup, unload and delete it.
</Note>
...
```bash
launchctl kickstart -k gui/$UID/ai.cimiclaw.ssh-tunnel
```

Stop the tunnel:

```bash
launchctl bootout gui/$UID/ai.cimiclaw.ssh-tunnel
```
