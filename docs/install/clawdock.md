...
- `~/.cimiclaw/.env` for env-backed provider keys and bot tokens
- `~/.cimiclaw/agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `~/.cimiclaw/cimiclaw.json` for behavior config

Use `clawdock-show-config` when you want to inspect the `.env` files and `cimiclaw.json` quickly. It redacts `.env` values in its printed output.
