...
- Persist `~/.cimiclaw` + `~/.cimiclaw/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.cimiclaw` state includes `cimiclaw.json`, per-agent
...
<Steps>
  ...
  <Step title="Create persistent host directories">
    ...
    ```bash
    mkdir -p /root/.cimiclaw/workspace

    # Set ownership to the container user (uid 1000):
    chown -R 1000:1000 /root/.cimiclaw
    ```
  ...
  <Step title="Configure environment variables">
    ...
    OPENCLAW_CONFIG_DIR=/root/.cimiclaw
    OPENCLAW_WORKSPACE_DIR=/root/.cimiclaw/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.cimiclaw
    ```
