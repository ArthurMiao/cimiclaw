...
- Persistent state lives on the host under `~/.cimiclaw` by default.
...
- It creates `~/.cimiclaw/cimiclaw.json` with `gateway.mode: "local"` if missing.
- It creates `~/.cimiclaw/.env` with `OPENCLAW_GATEWAY_TOKEN` if missing.
- For manual launches, the helper reads only a small allowlist of Podman-related keys from `~/.cimiclaw/.env` and passes explicit runtime env vars to the container; it does not hand the full env file to Podman.
...
Then open `http://127.0.0.1:18789/` and use the token from `~/.cimiclaw/.env`.
...
- **Config dir:** `~/.cimiclaw`
- **Workspace dir:** `~/.cimiclaw/workspace`
- **Token file:** `~/.cimiclaw/.env`
...
`cimiclaw.json`, per-agent `auth-profiles.json`, channel/provider state,
...
The manual launcher reads `~/.cimiclaw/.env` before finalizing container/image defaults, so you can persist these there.
...
- It reads `~/.cimiclaw/.env` as a runtime `EnvironmentFile` for values such as `OPENCLAW_GATEWAY_TOKEN`, but it does not consume the manual launcher's Podman-specific override allowlist.
...
- **Gateway start blocked (missing `gateway.mode=local`):** Ensure `~/.cimiclaw/cimiclaw.json` exists and sets `gateway.mode="local"`. `scripts/podman/setup.sh` creates this if missing.
