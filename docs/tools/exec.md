...
Request elevated mode 鈥?escape the sandbox onto the configured host path. `security=full` is forced only when elevated resolves to `full`.
</ParamField>
...
- `gateway`/`node` approvals are controlled by `~/.cimiclaw/exec-approvals.json`.
...
- No-approval host exec is the default for gateway + node. If you want approvals/allowlist behavior, tighten both `tools.exec.*` and the host `~/.cimiclaw/exec-approvals.json`; see [Exec approvals](/tools/exec-approvals#yolo-mode-no-approval).
...
- [Exec Approvals](/tools/exec-approvals) 鈥?approval gates for shell commands
- [Sandboxing](/gateway/sandboxing) 鈥?running commands in sandboxed environments
- [Background Process](/gateway/background-process) 鈥?long-running exec and process tool
- [Security](/gateway/security) 鈥?tool policy and elevated access
