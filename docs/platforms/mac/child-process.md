...
- The app installs a per-user LaunchAgent labeled `ai.cimiclaw.gateway`
  (or `ai.cimiclaw.<profile>` when using `--profile`/`OPENCLAW_PROFILE`; legacy `com.openclaw.*` is supported).
