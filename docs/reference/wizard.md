...
<Steps>
  <Step title="Existing config detection">
    - If `~/.cimiclaw/cimiclaw.json` exists, choose **Keep current values**, **Review and update**, or **Reset before setup**.
  ...
  <Step title="Model/Auth">
    ...
    - Auth profiles live in `~/.cimiclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.cimiclaw/credentials/oauth.json` is legacy import-only.
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    that agent's `auth-profiles.json` (for example
    `~/.cimiclaw/agents/<agentId>/agent/auth-profiles.json`, or the matching
    `$OPENCLAW_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
    is only a legacy import source.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.cimiclaw/workspace` (configurable).
...
- Stores it under `~/.cimiclaw/tools/signal-cli/<version>/`.
...
Typical fields in `~/.cimiclaw/cimiclaw.json`:
...
WhatsApp credentials go under `~/.cimiclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.cimiclaw/agents/<agentId>/sessions/`.
