...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.cimiclaw/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
exceeded`, and similar provider-shaped variants) â†?compact â†?retry.
