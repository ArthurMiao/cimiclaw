...
<AccordionGroup>
  ...
  <Accordion title="Environment note">
    ...
    <Warning>
    A key sitting only in `~/.profile` will not help a launchd/systemd daemon unless that environment is imported there as well. Set the key in `~/.cimiclaw/.env` or via `env.shellEnv` to ensure the gateway process can read it.
    </Warning>
