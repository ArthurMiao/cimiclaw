...
<AccordionGroup>
  ...
  <Accordion title="Environment availability for the daemon">
    If the Gateway runs as a managed service (launchd, systemd, Docker), `TOKENHUB_API_KEY` must be visible to that process. Set it in `~/.cimiclaw/.env` or via `env.shellEnv` so launchd, systemd, or Docker exec environments can read it.
