...
function createParams(baseEnv?: NodeJS.ProcessEnv) {
  return {
    configPath: "/tmp/openclaw-qa/cimiclaw.json",
...
describe("buildQaRuntimeEnv", () => {
  ...
  it("cleans startup temp roots when they are not preserved", async () => {
    ...
    await writeFile(path.join(tempRoot, "cimiclaw.json"), "{}", "utf8");
...
describe("qa bundled plugin dir", () => {
  ...
  it("resolves bundled plugins by manifest id when the directory name differs", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "kimi-coding", "cimiclaw.plugin.json"),
  ...
  it("uses a source bundled plugin when the built copy is missing CLI metadata", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "memory-core", "cimiclaw.plugin.json"),
    ...
    await writeFile(
      path.join(repoRoot, "extensions", "memory-core", "cimiclaw.plugin.json"),
      JSON.stringify({ id: "memory-core", kind: "memory" }),
      "utf8",
    );
    await writeFile(
  ...
  it("maps cli backend provider ids to their owning bundled plugin ids", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "openai", "cimiclaw.plugin.json"),
      JSON.stringify({
        id: "openai",
