...
`;
    ...
    expect(parsed.cimiclaw.emoji).toBe("💾");
    expect(parsed.cimiclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    expect(parsed.cimiclaw.emoji).toBe("📝");
    expect(parsed.cimiclaw.events).toEqual(["command"]);
    expect(parsed.cimiclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.cimiclaw.install[0].kind).toBe("bundled");
  });
...
homepage: https://docs.cimiclaw.ai/automation/hooks#session-memory
metadata:
  {
    "openclaw":
      {
