...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  await fs.mkdir(path.join(workspaceDir, ".cimiclaw"), { recursive: true });
  await fs.writeFile(
    path.join(workspaceDir, ".cimiclaw", "workspace-state.json"),
    `${JSON.stringify(
      {
        version: 1,
  ...
  const toolProbePath = path.join(workspaceDir, `.cimiclaw-live-tool-probe.${nonceA}.txt`);
...
describeLive("gateway live (dev agent, profile keys)", () => {
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.cimiclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
