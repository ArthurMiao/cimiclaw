...
export function registerDnsCli(program: Command) {
  ...
  dns
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(
        theme.heading(
          "Recommended config ($OPENCLAW_CONFIG_PATH, default ~/.cimiclaw/cimiclaw.json):",
        ),
      );
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in the active OpenClaw config ($OPENCLAW_CONFIG_PATH, default ~/.cimiclaw/cimiclaw.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
