...
describe("detectLinuxSdBackedStateDir", () => {
  it("detects state dir on mmc-backed mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/pi/.cimiclaw", {
      platform: "linux",
      mountInfo,
    });

    expect(result).toEqual({
      path: "/home/pi/.cimiclaw",
      mountPoint: "/",
      fsType: "ext4",
  ...
  it("returns null for non-mmc devices", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/user/.cimiclaw", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("uses resolved state path to select mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/tmp/openclaw-state", {
      platform: "linux",
      mountInfo,
      resolveRealPath: () => "/mnt/slow/openclaw/.cimiclaw",
    });

    expect(result).toEqual({
      path: "/mnt/slow/openclaw/.cimiclaw",
      mountPoint: "/mnt/slow",
      fsType: "ext4",
  ...
  it("returns null outside linux", () => {
    ...
    const result = detectLinuxSdBackedStateDir(path.join("/Users", "tester", ".cimiclaw"), {
      platform: "darwin",
      mountInfo,
    });
  ...
  it("escapes decoded mountinfo control characters in warning output", () => {
    ...
    const stateDir = `${mountRoot}/.cimiclaw`;
