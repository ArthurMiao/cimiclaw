...
export function readEmbeddedGatewayTokenForTest(
  ...
  return isEnvironmentFileOnlySource(command?.environmentValueSources?.cimiclaw_GATEWAY_TOKEN)
    ? undefined
    : normalizeOptionalString(command?.environment?.cimiclaw_GATEWAY_TOKEN);
}
