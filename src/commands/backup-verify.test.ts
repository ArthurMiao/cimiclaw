...
describe("backupVerifyCommand", () => {
  ...
  it("verifies a valid backup archive", async () => {
    ...
    try {
      ...
      const payloadArchivePath = `${archiveRoot}/payload/posix/tmp/.cimiclaw/state.txt`;
      await fs.writeFile(
        manifestPath,
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.cimiclaw",
            archivePath: `${rootName}/payload/posix/tmp/.cimiclaw`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    ...
    try {
      ...
      const stateArchivePath = `${archiveRoot}/payload/posix/tmp/.cimiclaw/state.txt`;
  ...
  it("rejects duplicate manifest and payload entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.cimiclaw/payload.txt`;
