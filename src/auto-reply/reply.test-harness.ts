...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".cimiclaw", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.OPENCLAW_STATE_DIR = path.join(home, ".cimiclaw");
    process.env.OPENCLAW_AGENT_DIR = path.join(home, ".cimiclaw", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".cimiclaw", "agent");
