...
function expectMalformedAgentAskUsesDefaults(agentAsk: unknown): void {
  ...
  expectFields(summary.ask, {
    requested: "off",
    host: "always",
    hostSource: "~/.cimiclaw/exec-approvals.json defaults.ask",
    effective: "always",
...
describe("exec approvals policy helpers", () => {
  ...
  it("explains stricter host security and ask precedence", () => {
    ...
    expectFields(summary.security, {
      ...
      host: "allowlist",
      effective: "allowlist",
      hostSource: "~/.cimiclaw/exec-approvals.json defaults.security",
    ...
    expect(summary.askFallback).toEqual({
      effective: "deny",
      source: "~/.cimiclaw/exec-approvals.json defaults.askFallback",
    });
  });
  ...
  it("attributes host policy to wildcard agent entries before defaults", () => {
    ...
    expectFields(summary.security, {
      host: "allowlist",
      hostSource: "~/.cimiclaw/exec-approvals.json agents.*.security",
    });
    expectFields(summary.ask, {
      host: "always",
      hostSource: "~/.cimiclaw/exec-approvals.json agents.*.ask",
    });
    expect(summary.askFallback).toEqual({
      effective: "deny",
      source: "~/.cimiclaw/exec-approvals.json agents.*.askFallback",
    });
  });
  ...
  it("avoids a duplicate default-agent scope when main only appears in approvals", () => {
    ...
    expectFields(snapshots[0]?.security, {
      host: "allowlist",
      hostSource: "~/.cimiclaw/exec-approvals.json agents.main.security",
    });
    expectFields(snapshots[0]?.ask, {
      host: "always",
      hostSource: "~/.cimiclaw/exec-approvals.json agents.main.ask",
    });
  });
