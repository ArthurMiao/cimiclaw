...
const STAGED_NPM_PROJECT_CONFIG_PREFIX = ".cimiclaw-install-hidden-npmrc-";
...
export async function installPackageDir(params: {
  ...
  try {
    ...
    stageDir = await fs.mkdtemp(path.join(installBaseRealPath, ".cimiclaw-install-stage-"));
  ...
  if (params.mode === "update" && (await pathExists(canonicalTargetDir))) {
    const backupRoot = path.join(installBaseRealPath, ".cimiclaw-install-backups");
