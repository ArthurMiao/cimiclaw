...
describe("runGatewayUpdate", () => {
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "cimiclaw.mjs"), "export {};\n", "utf-8");
  });
  ...
  async function createStableTagRunner(params: {
    ...
    const doctorKey = `${doctorNodePath} ${path.join(tempDir, "cimiclaw.mjs")} doctor --non-interactive --fix`;
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${doctorNodePath} ${path.join(tempDir, "cimiclaw.mjs")} doctor --non-interactive --fix`]: {
        stdout: "",
      },
    });
  ...
  it("bootstraps pnpm via npm when pnpm and corepack are unavailable", async () => {
    ...
    const { calls, runCommand } = createGitInstallRunner({
      ...
      doctorCommand: `${process.execPath} ${path.join(tempDir, "cimiclaw.mjs")} doctor --non-interactive`,
  ...
  it("uses npm-bootstrapped pnpm for dev preflight when pnpm and corepack are missing", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(tempDir, "cimiclaw.mjs")} doctor --non-interactive --fix`;
  ...
  it("runs dev preflight lint in constrained mode when explicitly enabled", async () => {
    ...
    expect(lintEnv[0]?.cimiclaw_LOCAL_CHECK).toBe("1");
    expect(lintEnv[0]?.cimiclaw_LOCAL_CHECK_MODE).toBe("throttled");
    expect(lintEnv[0]?.cimiclaw_OXLINT_SHARDS_SERIAL).toBe("1");
  });
  ...
  it("falls back to the cloned cwd when git root probing misses a fresh checkout", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(gitRoot, "cimiclaw.mjs")} doctor --non-interactive --fix`;
  ...
  it("runs doctor after global npm updates before reporting success", async () => {
    ...
    expect(result.steps.map((step) => step.name)).toContain("cimiclaw doctor");
    expect(doctorEnv?.cimiclaw_UPDATE_IN_PROGRESS).toBe("1");
    expect(doctorEnv?.cimiclaw_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBe("1");
  });
  ...
  it("fails global npm updates when post-update doctor fails", async () => {
    ...
    expect(lastStep?.name).toBe("cimiclaw doctor");
  ...
  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "openclaw");
    const staleDir = path.join(nodeModules, ".cimiclaw-stale");
  ...
  it("fails with a clear reason when cimiclaw.mjs is missing", async () => {
    await setupGitCheckout({ packageManager: "pnpm@8.0.0" });
    await fs.rm(path.join(tempDir, "cimiclaw.mjs"), { force: true });
    ...
    expect(result.steps.at(-1)?.name).toBe("cimiclaw doctor entry");
  });
