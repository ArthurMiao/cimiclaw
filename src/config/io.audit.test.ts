...
function createRenameAuditRecord(home: string) {
  return finalizeConfigWriteAuditRecord({
    base: createAuditRecordBase(path.join(home, ".cimiclaw", "cimiclaw.json")),
    result: "rename",
    nextMetadata: {
      dev: "12",
      ino: "13",
...
function readAuditLog(home: string): unknown[] {
  const auditPath = path.join(home, ".cimiclaw", "logs", "config-audit.jsonl");
  return fs
...
describe("config io audit helpers", () => {
  ...
  it('ignores literal "undefined" home env values when choosing the audit log path', async () => {
    ...
    expect(auditPath).toBe(path.join(home, ".cimiclaw", "logs", "config-audit.jsonl"));
  ...
  it("formats overwrite warnings with hash transition and backup path", () => {
    expect(
      ...
      "Config overwrite: /tmp/cimiclaw.json (sha256 prev-hash -> next-hash, backup=/tmp/cimiclaw.json.bak, changedPaths=3)",
    );
  });
  ...
  it("drops next-file metadata and preserves error details for failed writes", () => {
    const base = createAuditRecordBase("/tmp/cimiclaw.json");
