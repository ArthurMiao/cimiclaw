...
function createPlugin(
  ...
  writeJson(path.join(pluginDir, "cimiclaw.plugin.json"), {
    id: params.id,
...
function readBundledManifest(repoRoot: string, pluginId: string): Record<string, unknown> {
  return JSON.parse(
    fs.readFileSync(
      path.join(repoRoot, "dist", "extensions", pluginId, "cimiclaw.plugin.json"),
      "utf8",
    ),
...
describe("copyBundledPluginMetadata", () => {
  it("copies plugin manifests, package metadata, and local skill directories", () => {
    ...
    expect(
      fs.existsSync(path.join(repoRoot, "dist", "extensions", "acpx", "cimiclaw.plugin.json")),
    ).toBe(true);
    expect(
      fs.readFileSync(
  ...
  it("removes generated outputs for plugins no longer present in source", () => {
    ...
    writeJson(path.join(repoRoot, "dist", "extensions", "removed-plugin", "cimiclaw.plugin.json"), {
      id: "removed-plugin",
      configSchema: { type: "object" },
  ...
  it("removes stale dist outputs when a source extension directory no longer has a manifest", () => {
    ...
    writeJson(path.join(staleDistDir, "cimiclaw.plugin.json"), {
  ...
  it("removes non-packaged private QA plugin metadata unless private QA build is enabled", () => {
    ...
    expect(fs.existsSync(path.join(staleDistDir, "cimiclaw.plugin.json"))).toBe(true);
  ...
  it("preserves manifest-less runtime support package outputs and copies package metadata", () => {
    ...
    expect(
      fs.existsSync(
        path.join(repoRoot, "dist", "extensions", "image-generation-core", "cimiclaw.plugin.json"),
      ),
    ).toBe(false);
    expect(
      JSON.parse(
