...
describe("restart log conventions", () => {
  it("resolves profile-aware gateway logs and restart attempts together", () => {
    ...
    expect(resolveGatewayLogPaths(env)).toEqual({
      logDir: "/Users/test/.cimiclaw-work/logs",
      stdoutPath: "/Users/test/.cimiclaw-work/logs/gateway.log",
      stderrPath: "/Users/test/.cimiclaw-work/logs/gateway.err.log",
    });
    expect(resolveGatewayRestartLogPath(env)).toBe(
      `/Users/test/.cimiclaw-work/logs/${GATEWAY_RESTART_LOG_FILENAME}`,
    );
  });
  ...
  it("renders best-effort POSIX log setup with escaped paths", () => {
    ...
    expect(setup).toContain(
      "if mkdir -p '/Users/test'\\''s/.cimiclaw/logs' 2>/dev/null && : >>'/Users/test'\\''s/.cimiclaw/logs/gateway-restart.log' 2>/dev/null; then",
    );
    expect(setup).toContain("exec >>'/Users/test'\\''s/.cimiclaw/logs/gateway-restart.log' 2>&1");
  });
  ...
  it("renders CMD log setup with quoted paths", () => {
    ...
    expect(setup.quotedLogPath).toBe('"C:\\Users\\Test User/.cimiclaw/logs/gateway-restart.log"');
    expect(setup.lines).toContain(
      'if not exist "C:\\Users\\Test User/.cimiclaw/logs" mkdir "C:\\Users\\Test User/.cimiclaw/logs" >nul 2>&1',
    );
  });
});
