...
- Reports that require write access to trusted local state (`~/.cimiclaw`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Anyone who can modify `~/.cimiclaw` state/config (including `cimiclaw.json`) is effectively a trusted operator.
