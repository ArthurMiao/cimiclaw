...
if ! docker_e2e_wait_container_bash "$CONTAINER_NAME" 180 0.5 "source scripts/lib/cimiclaw-e2e-instance.sh; openclaw_e2e_probe_tcp 127.0.0.1 $PORT"; then
...
docker_e2e_docker_cmd exec "$CONTAINER_NAME" bash -lc "source /tmp/cimiclaw-test-state-env
