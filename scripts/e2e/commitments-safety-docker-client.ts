...
async function withStateDir<T>(name: string, fn: (stateDir: string) => Promise<T>): Promise<T> {
  ...
  const previousStateDir = process.env.cimiclaw_STATE_DIR;
  try {
    process.env.cimiclaw_STATE_DIR = root;
    return await fn(root);
  } finally {
    resetCommitmentExtractionRuntimeForTests();
    if (previousStateDir === undefined) {
      delete process.env.cimiclaw_STATE_DIR;
    } else {
      process.env.cimiclaw_STATE_DIR = previousStateDir;
    }
