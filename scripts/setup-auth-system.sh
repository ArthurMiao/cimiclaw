...
# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/cimiclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/cimiclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/cimiclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now cimiclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status cimiclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
