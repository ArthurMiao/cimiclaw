...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-cimiclaw.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.cimiclaw/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.cimiclaw/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".cimiclaw", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".cimiclaw", "external-plugins", "glueclaw", "cimiclaw.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".cimiclaw", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.OPENCLAW_CONFIG_PATH = "~/custom-cimiclaw.json5";
    process.env.OPENCLAW_TEST_HOME = priorIsolatedHome;
    process.env.OPENCLAW_STATE_DIR = path.join(priorIsolatedHome, ".cimiclaw");
    ...
    const copiedConfigPath = path.join(testEnv.tempHome, ".cimiclaw", "cimiclaw.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".cimiclaw", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".cimiclaw", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
