...
function stageLiveTestState(params: {
  ...
  let realStateDir = rawStateDir
    ...
    : path.join(params.realHome, ".cimiclaw");
  ...
  if (
    ...
    realStateDir === path.join(priorIsolatedHome, ".cimiclaw")
  ) {
    realStateDir = path.join(params.realHome, ".cimiclaw");
  }
  const tempStateDir = path.join(params.tempHome, ".cimiclaw");
