...
function waitForChatResult(params: {
  ...
  return new Promise((resolve, reject) => {
    if (params.signal?.aborted) {
      reject(new DOMException("CimiClaw tool call aborted", "AbortError"));
      return;
    }
    const timer = window.setTimeout(() => {
      cleanup();
      reject(new Error("CimiClaw tool call timed out"));
    }, params.timeoutMs);
    const onAbort = () => {
      cleanup();
      reject(new DOMException("CimiClaw tool call aborted", "AbortError"));
    ...
    unsubscribe = params.client.addEventListener((evt: GatewayEventFrame) => {
      ...
      if (payload.state === "final") {
        cleanup();
        resolve(extractTextFromMessage(payload.message) || "CimiClaw finished with no text.");
      } else if (payload.state === "aborted") {
        cleanup();
        reject(
          new DOMException(payload.errorMessage ?? "CimiClaw tool call aborted", "AbortError"),
        );
      } else if (payload.state === "error") {
        cleanup();
        reject(new Error(payload.errorMessage ?? "CimiClaw tool call failed"));
      }
    });
    function cleanup() {
...
export async function submitRealtimeTalkConsult(params: {
  ...
  try {
    ...
    if (!runId) {
      throw new Error("CimiClaw realtime tool call did not return a run id");
    }
    if (params.signal?.aborted) {
      abortRun();
      return;
