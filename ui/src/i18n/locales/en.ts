...
export const en: TranslationMap = {
  ...
  overview: {
    ...
    pairing: {
      ...
      mobileHint:
        "On mobile? Copy the full URL (including #token=...) from cimiclaw dashboard --no-open on your desktop.",
    ...
    cards: {
      ...
      modelAuthAttentionExpiredDesc: "{providers} — re-authenticate with cimiclaw models auth",
  ...
  login: {
    ...
    failure: {
      ...
      authRequired: {
        ...
        stepPaste:
          "Paste the token from cimiclaw dashboard --no-open or enter the configured password.",
        stepGenerate:
          "If no token is configured, run cimiclaw doctor --generate-gateway-token on the gateway host.",
      ...
      authFailed: {
        ...
        stepDashboard:
          "Run cimiclaw dashboard --no-open and open the fresh URL or paste its token.",
        stepReplace:
      ...
      pairing: {
        ...
        stepList: "Run cimiclaw devices list on the Gateway host.",
        stepApproveId: "Approve this request: cimiclaw devices approve {requestId}.",
      ...
      protocol: {
        ...
        stepDashboard:
          "Reopen the served dashboard with cimiclaw dashboard so the UI and Gateway come from the same install.",
        stepDevUi:
          "If using pnpm ui:dev, rebuild or restart the dev UI against the current checkout.",
        stepRestart:
          "Restart the Gateway after updating CimiClaw so it serves the current protocol.",
      },
      network: {
        title: "Could not connect",
        summary:
          "The browser could not complete the Gateway connection. Check the target and transport before retrying credentials.",
        stepGateway: "Confirm the Gateway is running with cimiclaw status or cimiclaw gateway run.",
        stepUrl:
          "Check the WebSocket URL and use wss:// when the Gateway is behind HTTPS/Tailscale Serve.",
        stepDashboard:
          "Reopen the dashboard with cimiclaw dashboard --no-open to recopy the current URL and auth details.",
      },
    },
  },
  chat: {
